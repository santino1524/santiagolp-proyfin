		...
		return args -> {
			if (userRepository.count() == 0) {
				User userInit = User.builder().email(Constants.DEFAULT_USER)
						.passwd(passwordEncoder.encode(Constants.DEFAULT_PASSWD)).name("Goku").surname("Administrador")
						.secondSurname("Tienda").dni("11111111P").phoneNumber("666666666")
						.role(UserRole.valueOf(UserRole.SELLER.name())).enabled(true).build();
